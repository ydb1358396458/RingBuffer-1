...
相较于普通的收发缓冲数组，使用 RingBuffer 提供的函数可更容易实现数据的读写、查询、删除等操作，不需要编写复杂的地址记录、读写计数等代码，减少了数据指针溢出、数据被覆盖的风险；同时 RingBuffer 提供了可自定义分隔关键字的插入、查找等功能，运用在嵌入式的串口收发中可以实现多段数据的储存，段落之间分隔独立，降低硬件时序阻塞带来的影响，降低串口对芯片实时响应的要求并提升了数据传输的可靠性；
...
5. 此时可用 RingBuffer 提供的各种函数对新建的缓冲区进行读写等操作；
...
## 更新
2021.01.19 第一次提交<br>
2021.01.24 增加匹配字符查找函数<br>
2021.01.27 重制匹配字符查找功能，现已支持8位到32位关键词查询<br>
2021.01.28 复位函数修改为删除函数、增加关键词插入函数（自适应大小端）<br>
2021.01.30 修复了String读写函数的小概率指针溢出错误<br>
2021.06.29 修复了 Ring_Buffer_Write_String 的参数类型错误，修复了 Ring_Buffer_Write_Byte 无法写数组最后一位的问题<br>