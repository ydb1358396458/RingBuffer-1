 ...
 * \file ring_buffer.c
 * \brief 简易环形缓冲的实现
 * \details 可以向环形缓冲区快速的储存/读取数据，无需手动记录数据接收量即可准确的读出所有数据；
 *          无需手动清空数据缓存区，只要将上次接收的数据读取出来，缓冲区即可准备好接收下一段数据；
 * \author netube_99\netube@163.com
 * \date 2021.06.29
 * 
 * 2021.01.19 第一次提交
 * 2021.01.24 增加匹配字符查找函数
 * 2021.01.27 重制匹配字符查找功能，现已支持8位到32位关键词查询
 * 2021.01.28 复位函数修改为删除函数、增加关键词插入函数（自适应大小端）
 * 2021.01.30 修复了String读写函数的小概率指针溢出错误
 * 2021.06.29 修复了 Ring_Buffer_Write_String 的参数类型错误
